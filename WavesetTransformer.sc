WavesetTransformer {
	...
	var <>subBuf, <>mainOut, <>out, <>waves;

	var <>startModFreq, <>startAmt, <>speed2, <>distance, <>speedChangeModulo, <>plusAmount, <>speedMod, <>speedSwapModulo,;

	var <>waveSetAmount, <>multiplier, <>repeats, <>baseSpeed, <>shouldSwap, <>panAmt, <>inst;
	...
	var <>breakPointSet, <>breakPoint, <>decBreak,<>breakAmount;

	*new {| subBufNum, limiter, buf, waves, files  |
        ^super.new.init(subBufNum, limiter, buf, waves, files);
    }

    init { | subBufNum, limiter, buf, waves, files  |
		var buf = Buffer.alloc(s, 512,bufnum:subBufNum);

		SynthDef(\eq, {

			var limiter = Limiter.ar(In.ar(this.out, 2), 0.85);

			Out.ar(this.mainOut, limiter);
		}).add;
	...
	newWaves{
		this.inst = this.subBuf.sendCollection(wave);
	}
	...
	go {|length = 2|
		^Routine({
			length.do({arg j;
				...
				amount = this.currSet.xings.size -  (this.waveSetAmount);
				amount.do({arg i;
					...
					if(this.shouldTransferOne()){
						if(this.transferOneAmt > 0){
							...
							if(numWs > (this.transferSetOne.xings.size - 1)){
								numWs = (this.transferSetOne.xings.size - 1) - this.waveSetAmount();
							};
					...
					if(this.shouldTransferTwo){
						if(this.transferTwoAmt > 0){
							...
							if(numWs > (this.transferSetTwo.xings.size - 1)){
								numWs = (this.transferSetTwo.xings.size - 1) - this.waveSetAmount;
							};
					...
					if(this.shouldInterleaveOne()){
						if(this.interleaveOneAmt > 0){
							...
							if(numWs > (this.leaveOneSet.xings.size - 1)){
								numWs = (this.leaveOneSet.xings.size - 1) - this.waveSetAmount();
							};
					...
					if(this.shouldInterleaveTwo){
						if(this.interleaveTwoAmt > 0){
							...
							if(numWs > (this.leaveTwoSet.xings.size - 1)){
								numWs = (this.leaveTwoSet.xings.size - 1) - this.waveSetAmount();
							};
					...
					ev.length = ev.length*this.multiplier;

					if( (i%2 == 0 && this.shouldSwap()), {
						ev.length = (endFrame - startFrame)/this.multiplier();
					});
					...
					if( ((i%this.normalizeAmount) == 0) && this.shouldNormalize){
						if(ev.wsAmp > (this.currSet.avgAmp*this.normalizeThres)) ){
					...
					if(this.shouldDelete){
						if(this.deleteAmt > 0){
							...
							this.deleteAmt = self.deleteAmt - 1;
					...
					if(  (ev.wsAmp < (this.currSet.avgAmp*this.shrinkAmt)) && this.shouldShrink() ){
						...
						if(i !== (this.currSet.xings.size - (this.waveSetAmount + 1)), {
